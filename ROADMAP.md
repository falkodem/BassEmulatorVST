...
- [x] Убрать бессрочное удержание последнего F₀: сбрасывать бас на новой атаке, принимать только новые кадры PESTO и ограничить удержание при потере confidence 30 мс. Передавать cache между последовательными инференсами, а не в момент постановки кадра в очередь. Убрать файловый debug-вывод из аудиопотока. Слуховая проверка остаётся в пункте ниже.
- [x] Собрать macOS arm64 VST3 с временно подставленным экспортом `models/20260706_110636/pesto.onnx`: бандл загружается из другого каталога и проходит проверку подписи. Выбор рабочей модели и проверка звука в Reaper остаются открытыми.
- [ ] Собрать VST3 на Windows и проверить в Reaper на 44,1 кГц: слуховую точность атак и переходов, отсутствие застывшего старого питча, задержку и PDC.
