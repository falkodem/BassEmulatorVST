"""LossWeighting callbacks. Vendored from pesto-full src/callbacks/loss_weighting.py.

Only base `LossWeighting` is used by our setup — `WarmupLossWeighting` and
`GradientsLossWeighting` are kept in case we want them later.
"""
...
# pytorch-lightning re-exports the same Callback class under both names; we use
# the package name we'll install (`pytorch_lightning`) instead of upstream's
# `lightning.pytorch`.
...
class LossWeighting(pl.Callback):
    ...
    def on_train_batch_end(self,
                           ...
                           batch: Any,
                           batch_idx: int) -> None:
        pl_module.log_dict({f"hparams/{k}_weight": v for k, v in self.weights.items()},
