...
Usage:
    ...
    poetry run python -m ml.finetune_pesto.train --epochs 5 --lr 5e-6
...
What it does:
 ...
 5. To deploy in plugin: re-run `ml/utils/export_pesto_onnx.py --model-name <ckpt>`
"""
import argparse
import logging
import os
...
# add project root to path so `ml.finetune_pesto.X` imports work
ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT))

from ml.finetune_pesto.config import TrainConfig
from ml.finetune_pesto.streaming_datamodule import GuitarStreamingDataModule
from ml.finetune_pesto.vendor.networks.resnet1d import Resnet1d
from ml.finetune_pesto.vendor.pesto_module import PESTO, PitchShiftCQT
from ml.finetune_pesto.vendor.losses import CrossEntropyLoss, ShiftCrossEntropy, PowerSeries
from ml.finetune_pesto.vendor.loss_weighting import GradientsLossWeighting, LossWeighting
from ml.finetune_pesto.vendor.pesto_module import nn  # for transforms list
...
def main() -> int:
    ...
    log.info("  poetry run python ml/utils/export_pesto_onnx.py --model-name %s "
